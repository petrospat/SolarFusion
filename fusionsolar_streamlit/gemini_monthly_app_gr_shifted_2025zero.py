...
# gemini_monthly_app_gr_shifted_2025zero.py
# Streamlit app for Huawei FusionSolar Northbound API
# Greece plant: PLANT_TZ = "Europe/Athens" and +1 month shift (Option A).
# New: Year-aware budget & Actual-zeroing rule for 2025 (Jan–Sep = 0), and UI locked to 2025+.
...
# ----------------------- Page setup -----------------------
st.set_page_config(page_title="FusionSolar – GR Plant (2025 policy)", layout="wide")
...
def get_budget_df(year: int) -> pd.DataFrame:
    ...
    Enforce min year = 2025 (park start). Budget values can be adjusted here if needed.
    """
    if year < 2025:
        year = 2025
...
st.title("☀️ FusionSolar – GR Plant (2025 policy)")
...
with tab1:
    if st.button("Refresh Monthly", help="Fetch monthly KPIs for the selected year") and can_run:
        ...
        with st.spinner("Connecting and loading monthly KPIs..."):
            ...
            st.caption("Note: Park operations started in 2025. Actual production is set to 0 for Jan–Sep 2025.")
            c1, c2 = st.columns(2)
...
st.info("Plant timezone set to Europe/Athens, data months shifted by +1, and 2025 Jan–Sep Actual set to 0. Year selector limited to 2025+.")

# gemini_monthly_app_gr_shifted_2025zero_budget.py
# Streamlit app for Huawei FusionSolar Northbound API
# Greece plant: Europe/Athens; data months shifted by +1 (Option A)
# Policy: For year 2025, Actual_kWh = 0 and Budget_kWh = 0 for Jan–Sep
# UI: Year selector limited to 2025+
...
# ----------------------- Page setup -----------------------
st.set_page_config(page_title="FusionSolar – GR Plant (2025 zero budget)", layout="wide")
...
def post_with_backoff(session: requests.Session, url: str, json_payload: dict,
    ...
    """POST with exponential backoff and jitter. Does not call Streamlit UI helpers.

    Returns parsed JSON on success or raises on fatal failures.
    """
    last_resp = None
    for attempt in range(max_retries):
        ...
        if resp.status_code == 200:
            try:
                ...
                return resp.text
            # Application-level rate-limit (failCode 407) — retry with backoff
        ...
        # Non-200 HTTP status: backoff and retry
        sleep_for = min(max_sleep, base_sleep * (2 ** attempt)) + random.uniform(0, jitter)
        time.sleep(sleep_for)

    # Exhausted retries — raise with last response details where reasonable
...
def get_logged_in_client(secrets: dict, ttl_seconds: int = 600) -> Tuple[Optional[HuaweiClient], str]:
    """Return a cached, logged-in HuaweiClient from `st.session_state` when possible.

    - Reuses cached client if created within `ttl_seconds`.
...
def get_budget_df(year: int) -> pd.DataFrame:
    """Return the monthly budget table for the given year.
    - Enforce min year = 2025 (park start).
    - For 2025 ONLY: set Budget_kWh = 0 for Jan–Sep.
    """
    if year < 2025:
        year = 2025
    months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
    # Base budget targets (edit if you have a revised plan)
    targets = [117077, 89742, 140573, 172775, 177950, 186287, 197265, 190014, 168524, 132649, 86079, 82732]

    if year == 2025:
        # Zero budget Jan–Sep (indices 0..8)
        for i in range(0, 9):
            targets[i] = 0
...
def pick_metric(row: pd.Series) -> float:
    # Use the confirmed working key ('inverterYield')
    for k in ["dataItemMap.inverterYield", "dataItemMap.PVYield", "dataItemMap.inverter_power", "dataItemMap.ongrid_power"]:
        if k in row.index:
...
def get_device_list_cached(base_url: str, station_code: str, xsrf_token: str, verify_ssl: bool) -> List[dict]:
    ...
    # --- FIX for nested list/dict handling (addresses list.get error) ---
    rows = []
    if isinstance(j, dict):
        # 1. Get the 'data' field. It might be a dict (with 'list') or a list directly.
        data_field = j.get("data")
        
        if isinstance(data_field, dict):
            # 2. Case: {"data": {"list": [...]}}
            rows = data_field.get("list", [])
        elif isinstance(data_field, list):
            # 3. Case: {"data": [...]}
            rows = data_field
    elif isinstance(j, list):
        # 4. Handle raw list response
        rows = j
    # --- END FIX ---
...
st.title("☀️ FusionSolar – GR Plant (2025 zero budget)")
col_top1, col_top2, col_top3 = st.columns(3)
show_raw = col_top1.checkbox("Show raw JSON (debug)", value=False)
# Lock to 2025+
year = col_top2.number_input(
    "Year",
    min_value=2025,
    max_value=max(2025, datetime.now().year + 1),
    value=max(2025, datetime.now().year),
    step=1,
)
...
with tab1:
    if st.button("Refresh Monthly", help="Fetch monthly KPIs for the selected year") and can_run:
        ...
        with st.spinner("Connecting and loading monthly KPIs..."):
            ...
            if not j_mon or not isinstance(j_mon, dict) or not j_mon.get("success"):
                st.warning("Monthly API returned empty or non-successful response.")
                st.write("Response:", j_mon)
                if show_raw:
            ...
            # --- Year-aware budget & Actual fill (2025 Jan–Sep = 0) ---
            df = get_budget_df(int(year))
            df['Actual_kWh'] = 0.0

            for m in range(1, 13):
                rm = tmp.loc[tmp['month'] == m]
                # Zero Actual for Jan–Sep 2025
                if int(year) == 2025 and m <= 9:
                    df.at[m-1, 'Actual_kWh'] = 0.0
                    continue
                if rm.empty:
                    continue
                df.at[m-1, 'Actual_kWh'] = pick_metric(rm.iloc[0])

            # Prorate budget for the current month (based on plant timezone)
            try:
                now = pd.Timestamp.now(tz=PLANT_TZ)
            except Exception:
                now = pd.Timestamp.now()

            # If viewing the selected year in the current year, prorate the current month's budget
            if int(year) == now.year:
                m_idx = now.month - 1
                try:
                    days_in_month = calendar.monthrange(now.year, now.month)[1]
                    days_passed = now.day
                    if days_in_month > 0 and days_passed < days_in_month:
                        factor = days_passed / days_in_month
                        df.at[m_idx, 'Budget_kWh'] = df.at[m_idx, 'Budget_kWh'] * factor
                        st.info(f"Prorated budget for {df.at[m_idx, 'Month']} ({days_passed}/{days_in_month} days): factor={factor:.3f}")
                except Exception:
                    # Fallback: do not prorate if any error occurs determining days
                    pass

            # KPIs
            current_month_idx = min(12, max(1, int(now.month)))
            ...
            st.caption("Note: Park operations started in 2025. Actual & Budget set to 0 for Jan–Sep 2025.")
            c1, c2 = st.columns(2)
            ...
            with st.expander("Raw Data Table"):
                # Build a display DataFrame with Year, Month, Budget_kWh, Actual_kWh, Delta
                temp = df.copy()
                # Ensure numeric columns exist
                if 'Budget_kWh' not in temp.columns:
                    temp['Budget_kWh'] = 0.0
                if 'Actual_kWh' not in temp.columns:
                    temp['Actual_kWh'] = 0.0
                temp['Budget_kWh'] = pd.to_numeric(temp['Budget_kWh'], errors='coerce').fillna(0.0)
                temp['Actual_kWh'] = pd.to_numeric(temp['Actual_kWh'], errors='coerce').fillna(0.0)
...
st.info("Europe/Athens timezone, +1 month shift; 2025 Jan–Sep both Budget & Actual set to 0; year selector limited to 2025+.")